...
if ParamControl.Curve_Fitting_or_Not
    CurveFitting_OverwriteControlParameters;
end

ParamControl.Laser_case = 1;
SP_SSB_Simulator_System_Param_Init;
PCA_Param_Init;

%% Optimization
ParamOpt.FFE_tap_range = 41:2:51;
ParamOpt.Vbias_over_Vpi_range = 0.3:0.05:0.35;
ParamOpt.Vpi_range = 2:6;
...
for i4 = 1:length(ParamOpt.Vbias_over_Vpi_range)
    ...
    ParamOpt.Vpp_over_Vpi_range = 0.3-0.01:0.05:(ParamSys.Vbias_over_Vpi*2-0.01);
    %*********************************************************************
    for i3 = 1:length(ParamOpt.Vpp_over_Vpi_range)
        ...
        ParamOpt.CSPR_larger_than_threshold = 1;
        for i2 = 1:length(ParamOpt.FFE_tap_range)    
            ...
            for i1 = 1:ParamOpt.num_trial
                ...
                while ParamPhysicalModel.BER_avg > 0.45 % let's try again
                    idx = idx +1;
                    if ParamPhysicalModel.Measured_CSPR < 10 || idx >10
                        ParamOpt.CSPR_larger_than_threshold = 0;
                        break;
                    end
                    SP_SSB_PhysicalLayerSimulator;
                end
                if ParamOpt.CSPR_larger_than_threshold == 0
                    break;
                end
...
title('LaserPwr = 10 dBm, LineWidth = 5 MHz');
