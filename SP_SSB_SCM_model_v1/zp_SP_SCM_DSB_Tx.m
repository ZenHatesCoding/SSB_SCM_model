function [Tx_Time_Data,Tx_Sequence,ParamPS] = zp_SP_SCM_DSB_Tx(ParamControl,ParamSig,ParamDAC,ParamFib)
    %% Tool Func
    ...
    SC_Offset(1) = SC_BW(1)/2-sum(SC_BW)/2;
    ...
    %% Dummy Sequence
    [Tx_Time_Data_dummy,Tx_Sequence_dummy,ParamPS_dummy] = zp_SP_PAMx_Tx(ParamControl,ParamDAC, ParamSig,1,2); 
    ...
    for idx = 1:num_SC
        [Tx_Time_Data_SC,Tx_Sequence_SC,ParamPS] = zp_SP_PAMx_Tx(ParamControl,ParamDAC, ParamSig,idx,ParamSig.SC(idx));
              
        if ParamControl.TxDSP_practical_implementation_or_Not == 0
    ...
    %% pwr_norm
    Tx_Time_Data = real(pwr_normalization(Tx_Time_Data));
    ...
    if ParamControl.VSB_or_Not
        ...
        Tx_Time_Data = hilbert(Tx_Time_Data);
        if ParamControl.Preemphasis_or_Not
            switch ParamControl.DAC_LPF_option
                case 1
    ...
    %% Plot Tx side signal at DAC rate
    if ParamControl.Plot_Spectrum_or_Not
        figure;
