%% By ZP 05/31/2018
% for SCM only
function [BER_X,SNR_X] = zp_SP_SC_PAMx_DSB_Rx(ParamControl,ParamRxDSP,ParamSig,ParamADC,ParamPS,ParamFib,Rx_I,Tx_Sequence_SC, SC_idx, SE)    
    %% Tool Func
...
%     ParamTR.rollOff = ParamSig.roll_off;
%     ParamTR.originalLengthBeforeImposingSFO = length(Tx_Sequence_X);
%     [Tx_Sequence_X a b] = NDAandFF_TimingErrorEstimation_And_Interpolation(Tx_Sequence_X.',2e4, ParamTR);
...
%         Tx_Sequence_X = resample(Tx_2sps_X,1,2).';  
    end
    ...
    if ParamControl.Plot_Constellation_or_Not
        ...
        scatterplot(Rx_Symbols_X_comp(1:1e4));
